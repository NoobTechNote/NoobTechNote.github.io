...
> 常常 JSON payload 傳錯導致 crash 或 API response 都好幾秒，News List 頁面等 6 秒
...
例如，當發生錯誤時，可以提供比簡單的 API 響應快照更多的信息。這意味著使用 API 可觀察性工具，您的工程師將能夠更快地解決問題。
...
所有詳細信息。它們可以是結構化或非結構化的代碼行，其中包含您要查找的所有詳細信息。
...
痕跡是一種流動形式，通過它可以很容易地看到和分析活動背後的整個歷史。

由於可以記錄系統的整個框架，因此跟踪被廣泛用於確定可觀察性。通過痕蹟的具體點，您可以識別並糾正錯誤。
...
一般來說，給出規律時間間隔行為信息的是數學數據。數據可以以圖表的形式標註，便於追踪系統的高點和低點。

指標的使用一般用於業務和統計相關信息，以追踪頻率。
...
print log 很簡單，在分散式系統，就很難只依靠 tail、grep、awk 來從日誌中挖掘信息了，
...
不該將密碼，銀行賬號，身份證件這些敏感信息打到日誌裡

- 避免引用慢操作:

日誌中打印的信息應該是上下文中可以直接取到的，如果當前上下文中根本沒有這項數據，
需要專門調用遠程服務或者從數據庫獲取，又或者通過大量計算才能取到的話，那應該先考慮這項信息放到日誌中是不是必要且恰當的。

- 避免打印追踪診斷信息:

日誌中不要打印方法輸入參數、輸出結果、方法執行時長之類的調試信息。

- 避免誤導他人:

[JAVA 中不要用 e.printStackTrace()](https://blog.51cto.com/u_15127511/3436116)，e.printStackTrace() 語句要產生的字符串記錄的是堆棧信息，太長太多，內存被填滿了！
...
這是因為日誌是非結構化數據，一行日誌中通常會包含多項信息，如果不做處理，
...
| 知識產權 | 14.123.255.234                                                                                                 |
| 用戶名   | 無效的                                                                                                         |
| 約會時間 | 19/二月/2020:00:12:11 +0800                                                                                    |
| 方法     | 得到                                                                                                           |
| 網址     | /index.html                                                                                                    |
| 協議     | HTTP/1.1                                                                                                       |
| 地位     | 200                                                                                                            |
| 尺寸     | 1314                                                                                                           |
| 參考     | https://icyfenix.cn                                                                                            |
| 代理人   | Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.163 Safari/537.36 |
...
開始時點、執行時長等信息，每次開始調用服務前都要先埋入一個調用記錄，這個記錄稱為一個"跨度"(Span)。
...
- 基於日誌的追踪的思路是將 Trace、Span 等信息直接輸出到應用日誌中，然後隨著所有節點的日誌歸集過程匯聚到一起，
  再從全局日誌信息中反推出完整的調用鏈拓撲關係。

  日誌追踪的代表產品是 Spring Cloud Sleuth，下面是一段由 Sleuth 在調用時自動生成的日誌記錄，
  可以從中觀察到 TraceID、SpanID、父 SpanID 等追踪信息。
...
> ![Pinpoint](./ch10/10-6.png)

#### 基於邊車代理的追踪(Sidecar-Based Tracing)

- 服務網格
  Envoy 是專為大型現代 SOA（面向服務架構）架構設計的 L7 代理和通信總線，體積小，性能高。它的誕生源於以下理念：
...
指標是數據的數字表示。它包括名稱、標籤、值、發生時間等值，並傳達有關應用程序組件的信息。
