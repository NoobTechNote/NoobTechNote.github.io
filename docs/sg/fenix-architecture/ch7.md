...
在分析网关的性能时，通常只能进行定性分析，要量化地比较哪种网关性能最好、快多少是很困难的。就像我们都认为 Chrome 比 IE 快，但具体情境下快多少就很难明确说明。
...
客戶端負載平衡器與服務本身是 process 內通訊，而代理負載平衡器與服務本身是網絡通訊，数据要经过打包拆包、计算校验和、维护序列号等步骤。

然而，Kubernetes 保证同一个 Pod 中的容器不会跨越不同的节点，这些容器共享同一个网络名称空间，所以代理均衡器与服务实例的交互实际上是对本机回环设备的访问，所以它们比客户端负载均衡器在性能上更高效且稳定。

代理負載均衡器（Proxy Load Balancers）的优点如下：

1. 不再受编程语言的限制，可以发展一个支持多种编程语言的通用的代理均衡器，
2. 避免了客户端均衡器的稳定性影响到服务进程的稳定等问题。
3. 在服务拓扑感知方面代理均衡器也要更有优势。由于边车代理接受控制平面的统一管理，当服务节点拓扑关系发生变化时，控制平面就会主动向边车代理发送更新服务清单的控制指令，这避免了此前客户端均衡器必须长期主动轮询服务注册中心所造成的浪费。
4. 在安全性、可观测性上，由于边车代理都是一致的实现，有利于在服务间建立双向 TLS 通信，也有利于对整个调用链路给出更详细的统计信息。

总之，通过同一个 Pod 的独立容器实现的代理均衡器是目前最理想的处理微服务集群内部流量的方式，尽管服务网格本身还不够成熟，对操作系统、网络和运维方面的知识要求较高，但相信随着时间的推移，未来这将会成为微服务的主流通信方式。

### 7.3.3 地域與區域（Regions and Zones）

最後再談一個與負載平衡器相關的話題，地域與區域（Regions and Zones）。在微服務或雲端常見。

1. Region 地域：例如华北、东北、华东、华南等。全球或全国规模的大型系统服务集群通常会在多个地理区域部署，如本篇所举的案例。大型系统通过在不同的地理区域的机房部署来缩短用户与服务器之间的物理距离，以提高响应速度。小型系统通常只在异地容灾时才需要考虑地理区域。需要注意的是，不同地理区域之间沒有內網連接，所有流量都必須通過公共互聯網相連。如果微服務流量跨越不同地理區域，就和調用外部服務提供商的互聯網服務沒有什麼區別。因此，集群內部的流量不會跨越地理區域，服務發現和負載均衡器默認也不支持跨地理區域的服務發現和負載均衡。
...
如果你追求低延迟，譬如对时间有高要求的 SLA 应用，或者网络游戏服务器等，那就应该考虑将系统的所有服务都只部署在同一个区域中，因为尽管内网连接不受限于公网带宽，但毕竟机房之间的专线容量也是有限的，难以跟机房内部的交换机相比，延时也受物理距离、网络跳点数量等因素的影响。
